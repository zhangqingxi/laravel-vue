  ...
  <div>
    <BasicTable @register="registerTable" @fetch-success="onFetchSuccess">
      <template #toolbar>
        <Button
          type="primary"
          v-if="hasPermission(t('sys.menu.role_manage'), MenuPermissionModeEnum.ADD)"
          @click="handleCreate"
        >
          {{ t('sys.role.roleAdd') }}
        </Button>
      ...
      <template
        #footer
        v-if="hasPermission(t('sys.menu.role_manage'), MenuPermissionModeEnum.BATCH)"
      >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'status'">
          <Switch
            :loading="loading"
            :checkedChildren="t('sys.role.enable')"
            :checked="record.status === 1"
            :unCheckedChildren="t('sys.role.disable')"
...
<script lang="ts" setup>
  ...
  import { Tag, Switch, Button, Select, Flex } from 'ant-design-vue';
  import { columns, searchFormSchema } from './data';
  import { useI18n } from '@/hooks/web/useI18n';
  import { log } from '@/utils/log';
  import { useMessage } from '@/hooks/web/useMessage';
  import { MenuPermissionModeEnum, hasPermission } from '@/utils/permission';
  import { BatchUpdateRole, deleteRole, getRoleList, updateRole } from '@/api/sys/role';
  import { GetRoleInfoModel } from '@/api/sys/model/roleModel';
  ...
  const [registerTable, { reload, expandAll, getSelectRows }] = useTable({
    ...
    rowSelection: {
      type: 'checkbox',
      getCheckboxProps: (record: GetRoleInfoModel) => ({
        disabled: record.id === 1,
      }),
    },
    // 请求之前处理参数
  ...
  async function handleChangeStatus(record: any, status: any): Promise<void> {
    try {
      ...
      const result = await updateRole({ id: record.id, status: status });

      if (result) {
        record.status = status;
      }
    } catch (error) {
      log('update role', error);
    } finally {
      loading.value = false;
    }
  }
  ...
  async function handleBatch() {
    try {
      ...
      const result = await BatchUpdateRole({ ids: ids, type: unref(batchValue.value) });

      if (result) {
        reload();
      }
    } catch (error) {
      log('batch update role info', error);
    } finally {
      loading.value = false;
    }
  }
