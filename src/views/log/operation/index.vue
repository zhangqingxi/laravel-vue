<template>
  <div>
    <BasicTable @register="registerTable">
      ...
      <template
        #footer
        v-if="hasPermission(t('sys.menu.operation_log'), MenuPermissionModeEnum.DELETE)"
      >
...
<script lang="ts" setup>
  ...
  import { getOperationLogList, removeOperationLog } from '@/api/sys/log';
  ...
  const [registerTable, { reload }] = useTable({
    title: t('sys.log.operationLogList'),
  ...
  async function handleBatch() {
    try {
      ...
      const result = await removeOperationLog({ type: unref(batchValue) });
